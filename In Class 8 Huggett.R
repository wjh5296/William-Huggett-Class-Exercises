...
# Load data
blm_tweets <- read.csv("~/Documents/GitHub/TAD_2021/R lessons/blm_samp.csv", stringsAsFactors = F)
...
# It seems cutting the amount of documents in half reduced the best number or topics slightly, but the graph is a little hard to read. 

#### Do you think that the number of topics is linear in the number of documents??

# No, I think the trend would likely flatten as the number of documents got higher.
# A relationship seems to exist, but probalby not a linear one. 
