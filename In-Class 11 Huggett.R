...
moby_dfm <- dfm(data_char_mobydick, remove = stopwords('english'), remove_punct = T)

mobyFeatures <- topfeatures(moby_dfm, 75)
...
topDf2$term <- with(topDf2, reorder(term, -frequency))

ggplot(topDf2) + geom_point(aes( x=term, y=frequency)) + 
  theme(axis.text.x=element_text(angle=90, hjust=1))
